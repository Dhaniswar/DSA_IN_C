...
struct node *delete_from_the_end(struct node *tail){
    struct node *current=NULL, *prevoius=NULL;
    ...
    if(tail==NULL) {
        ...
        prevoius->next = tail->next;
        tail = prevoius;
        free(current);

        printf("Node Deleted Sucessfully from the end!\n");
        return tail;
