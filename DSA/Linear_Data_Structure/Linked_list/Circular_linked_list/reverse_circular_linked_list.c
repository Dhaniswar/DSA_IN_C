...
struct node *reverse_circular_linked_list(struct node *tail){
    struct node *previous=NULL, *current, *nextnode;
    ...
    if(tail==NULL) {
        ...
        printf("Node Deleted Sucessfully from the beginning!\n");
...
void display_reverse_Circular_linked_list(struct node *tail){
    struct node *temp=NULL;
...
int main() {
    ...
    tail = reverse_circular_linked_list(tail);

    printf("reversed circular linked list: \n");
    display_reverse_Circular_linked_list(tail);
