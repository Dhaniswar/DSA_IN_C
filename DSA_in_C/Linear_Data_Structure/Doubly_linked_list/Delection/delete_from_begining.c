...
Delete a node from the beginning in the Doubly linlked list
*/
...
// Delete a node from the beginning in the Doubly linlked list
struct node *delete_node_from_beginning(struct node *head, struct node **tail){

    struct node *temp;

    if(head==NULL){
        printf("Empty linked list, there is nothing to delete!\n");
        return head;
    }
...
int main() {
    ...
    head = delete_node_from_beginning(head, &tail);
